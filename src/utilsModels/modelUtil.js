 ...
 */
utilModel.findAll = async function (columns,all) {
       return await genericDAO.execute(`SELECT ${this.getColumnString(columns)} FROM ${this.table.name}  ${this.generalizarCriteria(all)}`);   
};
 ...
 * @returns {String}
 */
utilModel.generalizarCriteria = function (all) {
    if(all!== false){
        return  '';
    }else{
        return (this.columns.hasOwnProperty('estatusId')) ?`WHERE  ESTATUS_ID = ${ACTIVO}` :'';
    }
